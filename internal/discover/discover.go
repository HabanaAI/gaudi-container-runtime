...
func HasExternalInterfaces(sysfsRoot string, deviceIDs []string) bool {
	deviceCount := len(deviceIDs)
	extInfCount := 0

	for _, deviceId := range deviceIDs {
		pciPath := fmt.Sprintf("%s/sys/class/accel/accel%s/device/pci_addr", sysfsRoot, deviceId)

		data, err := os.ReadFile(pciPath)
		if err != nil {
			continue
		}
		pciAddr := strings.TrimSpace(string(data))

		netDevDir := fmt.Sprintf("%s/sys/bus/pci/devices/%s/net", sysfsRoot, pciAddr)
		if _, err := os.Stat(netDevDir); err == nil {
			extInfCount++
		}
	}

	if deviceCount == 0 || extInfCount < deviceCount {
		return false
	}
...
func AcceleratorPciID(acceleratorID string) (string, error) {
	modPath := fmt.Sprintf("%s%s/device/pci_addr", SysClassAccel, acceleratorID)
	content, err := os.ReadFile(path.Clean(modPath))
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			return "", nil
		}
		return "", fmt.Errorf("reading pci_addr file: %v", err)
	}
	return strings.TrimSpace(string(content)), nil
}
