...
func TestGetHabanaConfig(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.description, func(t *testing.T) {
			var cfg *habanaConfig
			getConfig := func() {
				hookConfig := config.DefaultConfig()
				cfg = getHabanaConfig(&hookConfig, tc.env, nil, tc.privileged)
			}
			if tc.expectedPanic {
				// panic
			}
			...
			if cfg == nil && tc.expectedConfig == nil {
				return
			}
			if cfg != nil && tc.expectedConfig != nil {
				if !reflect.DeepEqual(cfg.Devices, tc.expectedConfig.Devices) {
					t.Errorf("Unexpected habanaConfig (got: %v, wanted: %v)", cfg, tc.expectedConfig)
				}
				return
			}
			t.Errorf("Unexpected habanaConfig (got: %v, wanted %v)", cfg, tc.expectedConfig)
