...
func getDevicesFromEnvvar(env map[string]string, legacyImage bool) *string {
	...
	// Environment variable unset or empty or "void": return all
...
func getDevices(hookConfig *config.Config, env map[string]string, mounts []Mount, privileged bool, legacyImage bool) *string {
...
func getHabanaConfig(hookConfig *config.Config, env map[string]string, mounts []Mount, privileged bool) *habanaConfig {
	legacyImage := false
...
func getContainerConfig(hook config.Config) (config containerConfig) {
	var h HookState
