...
func exposeInterfaces(logger *slog.Logger, pid int, requestedDevs []string) error {
	...
	for _, hostIntf := range extInts {
		...
		err = netns.Do(func(_ ns.NetNS) error {
			if _, err := netlink.LinkByName(hostIntf); err == nil {
				foundExisting = true
			}
		...
		// If interface is already within the container namespace (container restart due to app crash etc.),
		// don't try to add it again.
		if err == nil && foundExisting {
			logger.Info("Interface already exists, skipping", "interface", hostIntf)
			continue
		}
		...
		defer func() {
			err = netlink.LinkDel(containerLink)
			if err != nil {
				logger.Error("Failed deleting temporary link", "interface", tempName, "error", err.Error())
			} else {
				logger.Info("Deleted temporary link", "interface", tempName)
			}
		}()
...
func execute(command string) error {
	...
	if err != nil {
		return fmt.Errorf("command failed: %s", string(content))
	}
	return nil
}
