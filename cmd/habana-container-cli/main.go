...
import (
	...
	"github.com/HabanaAI/habana-container-runtime/internal/netinfo"
...
func main() {
	...
	app := &cli.App{
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{
				Name:        "log-file",
				Usage:       "Log file location",
				Value:       "/var/log/habana-container-hook.log",
			...
			&cli.BoolFlag{
				Name:        "cdi",
				Usage:       "Indicate CDI mode to cli (without habana-container-runtime).",
				Value:       false,
				Destination: &cfg.useCDI,
			},
		},
...
func run(rootfs string, config config, logger *slog.Logger) error {
	...
	} else if config.useCDI {
		devIds := discover.DevicesIDs(devices.accelerators)

		err := netinfo.Generate(devIds, rootfs)
		if err != nil {
			return fmt.Errorf("handling macAddrInfo.json generation: %v", err)
		}
	}
...
func parseDevices(logger *slog.Logger, deviceFlag string) (availableDevices, error) {
	...
	if deviceFlag == "all" {
		...
		requestedIDs := strings.Split(deviceFlag, ",")
		for _, dev := range accelDevices {
			dID := deviceID(dev)
			for _, i := range requestedIDs {
				if i == dID {
...
func mountInContainer(pid int, hostPath, containerPath string) error {
	...
	if err := mountDevice.Run(); err != nil {
		err = removeFile.Run()
		if err != nil {
			return fmt.Errorf("error running the removeFile command %v: %v", removeFile.Args, errBuf.String())
		}
